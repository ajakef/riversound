...
#path = '/home/jake/Work/StreamAcoustics/DCEW/'
path = '/data/jakeanderson/2021_Boise_River/long_term/LG_DCEW/'
path_infrasound = path + 'mseed_all'
path_audible = ''

day1 = obspy.UTCDateTime('2021-04-07')
day2 = obspy.UTCDateTime('2021-04-29')
...
t, q = riversound.read_DCEW('/data/jakeanderson/2021_Boise_River/long_term/Discharge/LG_StreamHrlySummary_2021.csv')
plt.semilogy(t,q)
...
plt.yticks(np.log10(10*np.arange(1,5)), 10*np.arange(1,5))
plt.ylim(np.log10(np.array([10,40])))
...
plt.figure()
plt.plot(interp_times(times_infrasound, t, q), np.array(power_infrasound)*1000, 'b.-')
#plt.loglog(interp_times(times_audible, t, q), power_audible, 'r.-')
plt.xlabel('Discharge (m$^3$/s)')
plt.ylabel('Power (x 10$^{-3}$ Pa$^2$)')
plt.tight_layout()
#plt.legend(['Infrasound', 'Audible'])

## plot discharge vs infrasound and audible peak frequencies
plt.figure()
plt.plot(interp_times(times_infrasound, t, q), riversound.find_peak_freq(meanspec_infrasound, freqs_infrasound, 13), 'b.-')
