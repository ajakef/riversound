...
path_infrasound = '/data/jakeanderson/2021_Boise_River/long_term/WhitewaterPark/2016_mseed'
path_audible = ''

day1 = obspy.UTCDateTime('2016-04-20')
day2 = obspy.UTCDateTime('2016-05-26')
...
for i in range(n_days):
    ...
    if t1 < datetime.datetime.strptime('2016-08-01', '%Y-%m-%d'):
        sensor_file = 'RESP.XX.IS000..BDF.GEMV0.26.0_0016' # Rg: 1k and 2.2k in parallel
    else:
        sensor_file = 'RESP.XX.IS000..BDF.GEMV0.26.0_0005' # Rg: 2.2k
    tr_infrasound, tr_audible = riversound.read_infrasound_audible(t1, t2, path_infrasound, path_audible, infrasound_sensor_response_file = sensor_file)
    ...
    try: # check whether the data can be processed (i.e., make sure it isn't missing)
        tr_infrasound.filter('highpass', freq = 8, zerophase = True)
...
## notes:
## the data gap at 5/1 is real (approx 4/30 22:15 to 5/1 23:04), not sure why
## using a linear power scale in the spectrogram reproduces a figure from 2016 sent to Tim in an email
...
riversound.image(np.log10(5e-6 + medspec_infrasound), times_infrasound, freqs_infrasound, crosshairs = False, log_y = False)
plt.xlim(xlim[0], xlim[1])
