...
from riversound.play_data import play_tr, play_signal
...
def test_play_tr():
    ...
    play_tr(tr.copy())
    play_tr(tr.copy(), low_cutoff_freq = 10)
    play_tr(tr.copy(), speedup = 256)
    play_tr(tr.copy(), speedup = 256, low_cutoff_freq = 10)
    ...
    tr.resample(16000)
    play_tr(tr.copy())
    play_tr(tr.copy(), low_cutoff_freq = 400)
    play_tr(tr.copy(), speedup = 8)
    play_tr(tr.copy(), speedup = 8, low_cutoff_freq = 400)


def test_play_signal():
    t = np.arange(16000)/8000
    play_signal(np.sin(2*np.pi*t*440), 8000)
    play_signal(np.sin(2*np.pi*t*440), 16000)
